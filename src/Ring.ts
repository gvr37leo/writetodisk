...
enum SwapProgress{toB, toA, toB2}
    ...
    update(){
        if(this.state == State.rolling){
            ...
            if(step == 0){
                ...
                if(this.swapProgress == SwapProgress.toB2)this.swapIndex++
            ...
            if(this.writeCompletion >= 1){
                if(this.swapIndex == this.swaps.length)return
                this.state = State.rolling;
            ...
            if(floatEquals(this.writeCompletion, 0.5)){
                switch(this.swapRemember){
                    ...
                    case SwapProgress.toB2:
                        if(this.oldSwapIndex == this.swaps.length)return
...
var swapMachine = new Map<SwapProgress, SwapProgress>()
